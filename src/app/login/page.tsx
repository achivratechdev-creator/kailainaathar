...
export default function LoginPage() {
  ...
  // --- Actions ---
  const handleClose = () => {
    // Navigates back to the previous page in history (Home, Shop, etc.)
    router.back();
  };
  ...
  const handleSignUp = () => {
    // FIX: Use 'replace' to swap the current history entry.
    // This prevents the "loop" when clicking Close.
    ...
    <div className={`${poppins.variable} ${arima.variable} font-sans fixed inset-0 z-[9999] h-screen w-screen overflow-hidden bg-gray-50`}>
      ...
      <div className="absolute inset-0 z-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <button 
            onClick={handleClose}
            className="absolute top-4 right-4 z-20 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-700 hover:bg-gray-100 transition-all shadow-md hover:scale-110 active:scale-95 cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>

          {/* LEFT SIDE: Image Section (40% width) */}
          <div className="relative md:col-span-2 flex flex-col justify-center items-center overflow-hidden min-h-[120px] md:min-h-full bg-gray-100">
            <div 
          ...
          <div className="md:col-span-3 px-6 py-6 md:px-8 md:py-8 overflow-y-auto bg-white scrollbar-hide flex flex-col justify-center">
            <div className="max-w-md mx-auto w-full">
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-3">
                ...
                {/* Email Input */}
                <div>
                  <label className="block text-gray-700 font-poppins mb-1 text-xs">
                    Email Address *
                  </label>
                ...
                <div className="flex justify-end pt-1">
                  <button type="button" className="text-[#2e7d32] font-poppins font-medium text-xs hover:underline cursor-pointer">
                    Forgot Password?
                  </button>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`w-full bg-gradient-to-r from-[#2e7d32] to-[#66bb6a] text-white py-2.5 rounded-lg font-poppins font-medium hover:shadow-lg hover:shadow-[#2e7d32]/20 transition-all duration-300 active:scale-[0.98] mt-2 cursor-pointer ${loading ? "opacity-70" : ""}`}
                >
                ...
                <div className="grid grid-cols-2 gap-3 mb-2">
                  <button type="button" className="flex items-center justify-center gap-2 py-2 border-2 border-gray-200 rounded-lg font-poppins font-medium text-xs text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all cursor-pointer">
                ...
                <div className="text-center pt-1">
                  <p className="text-gray-600 font-poppins text-xs font-light">
                    ...
                    {/* FIX: Use handleSignUp to trigger replace() */}
                    <button 
                      type="button" 
